...
| Publicar uma versão exige um tag à mão | `git tag vX.Y.Z` dispara o workflow; o rascunho é publicado à mão de propósito. O número da versão em `tauri.conf.json` e `Cargo.toml` continua a ser bumped à mão — nada verifica que os dois concordam | #79 |
| Versão nunca sobe sozinha | 0.1.0 desde sempre. O updater compara versões, logo um release novo com a mesma versão não é oferecido a ninguém | #79 |
