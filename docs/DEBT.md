...
| Dois testes do engine falharam uma vez sob carga | `a_shared_worktree_is_adopted_after_a_restart_not_rebuilt` e `the_loser_of_the_agent_limit_never_builds`, numa corrida do workspace inteiro com outra compilação em paralelo; passam 3/3 isolados e 3/3 em workspace desde então. Mensagem perdida. Se voltar, é aqui que se olha — e o `wait_for` já tem 30s, portanto não deve ser timeout | 2026-08-28 |
